<template>
  <div>
    <h3 class="hkf_title text-center mb-4">Bank Ledger</h3>
    <div>
      <nav>
    ...
    <div>
      <form @submit.prevent="passLedger()" class="mt-5">
        <div class="form-row">
          ...
          <div class="form-group mx-sm-3 mb-2">
            <label class="hkf_text">Supplier/Client Name:</label>
            <input
              type="text"
              class="form-control"
...
export default {
  ...
  beforeRouteEnter(to, from, next) {
    next((vm) => {
      ...
      if (!User.hasLoggedIn()) {
        ...
        vm.$router.push({ name: "bank_ledger" });
      }
    });
  },
  data() {
  ...
  created() {
    axios.get("/api/bank_suppliers").then((res) => {
  ...
  methods: {
    ...
    passLedger() {
      ...
      axios
        .post("/api/bank_ledger/pass_ldg", requestData, {
          headers: {
