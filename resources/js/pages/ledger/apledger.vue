<template>
  <div>
    <h3 class="hkf_title text-center mb-4">Accounts Payable Ledger</h3>
    <div>
      <nav>
    ...
    <div>
      ...
      <div id="print-container">
        ...
        >
          ...
          <tbody>
            <tr v-for="(ap_ledger, index) in ap_ledgers" :key='index'>
              <td
                scope="row"
                style="
                  border: 1px solid black;
              ...
              <td
                style="
                  ...
                  text-align: center;
                "
                v-if="ap_ledger.due != 0 && ap_ledger.cash != null"
              >
                {{ ap_ledger.supplier_name }}
              </td>
              <td
                style="
                  border: 1px solid black;
                ...
                "
                v-else-if="
                  ap_ledger.paid != 0 && ap_ledger.cash != null && ap_ledger.balance != 0
                "
              >
                {{ ap_ledger.product_name }} purchase (Cash),{{ ap_ledger.supplier_name }}
              </td>
              <td
                style="
                  border: 1px solid black;
                ...
                "
              >
                {{ ap_ledger.product_name }} purchase (Cash)
              </td>
              <td
                style="
                  border: 1px solid black;
                  ...
                  text-align: center;
                "
                v-else-if="ap_ledger.paid != 0 && ap_ledger.bank != null"
              >
                {{ ap_ledger.supplier_name }}
              </td>
              ...
              <td
                style="
                  ...
                  text-align: center;
                "
                v-if="ap_ledger.return_amount != 0"
              >
                {{ ap_ledger.product_name }} return
              </td>
              <td
                style="
                  border: 1px solid black;
              ...
              <td
                style="
                  ...
                  text-align: center;
                "
                v-if="Math.abs(ap_ledger.due) != 0"
              >
                {{ Math.abs(ap_ledger.due) }}
              </td>
              <td
                style="
                  border: 1px solid black;
                  ...
                  text-align: center;
                "
                v-else-if="Math.abs(ap_ledger.due) == 0"
              ></td>
              <td
                style="
                  border: 1px solid black;
...
export default {
  ...
  created() {
    axios.get("/api/purchase_suppliers").then((res) => {
    ...
    axios
      ...
      .then((res) => {
        ...
        res.data.forEach((item) => {
          if (parseInt(item.due) != 0) {
            this.allLedgers.push(item);
          }
        });
      })
      .catch((err) => {
  ...
  computed: {
    ...
    filterSupplier() {
      return this.allSuppliers.filter((product) => {
        ...
        let supplierLedger = product.supplier_name.toUpperCase();
  ...
  methods: {
    ...
    assignToSearch2(item) {
      this.supplier_search = item.supplier_name;

      this.reference_list_style = true;
      this.selectedSupplier = item.supplier_name;
    },
    passLedger() {
      const requestData = {
      ...
      axios
        .post("/api/ap_ledger/pass_ldg", requestData, {
          headers: {
