<template>
  <div>
    <h2 class="text-center mb-4 hkf_title">Receipt Journal</h2>
        ...
        <tbdoy v-for="(receipt_journal, index) in receipt_journals" :key="index">
          ...
          <tr style="padding: 1rem; border: 1px solid black; border-collapse: collapse">
            ...
            <td
              ...
              "
            >
              {{ parseFloat(receipt_journal.product_amount).toFixed(2) }}
            </td>
            <td
              style="
                padding: 1rem;
          ...
          <tr style="padding: 1rem; border: 1px solid black; border-collapse: collapse">
            ...
            <td
              style="
                ...
                width: 24%;
              "
              v-if="receipt_journal.bank != null"
            >
              Bank
            </td>
            <td
              style="
                ...
                width: 24%;
              "
              v-if="receipt_journal.cash != null"
            >
              Cash
            </td>
            <td
...
export default {
  ...
  beforeRouteEnter(to, from, next) {
    next((vm) => {
      ...
      if (!User.hasLoggedIn()) {
        ...
        vm.$router.push({ name: "sales_journal" });
      }
    });
  },
  data() {
  ...
  created() {
    ...
    if (User.getGuard() == "admin") {
      this.isAdmin = true;
      axios
        .get("/api/fetchProjectsReceiptJournal")
        .then((res) => {
  ...
  methods: {
    dateAccounts() {
      ...
      axios
        .post("/api/receipt_journal/daily_journal", data, {
          headers: {
  ...
  computed: {
    selectDate() {
      axios
        .get("/api/receipt_journal/daily_journal/" + this.sendDate)
        .then((res) => {
          this.receipt_journals = [];
          this.empty = false;
          res.data.forEach((item) => {
    ...
    chooseProject() {
      ...
      axios
        .get("/api/fetchProjectsReceiptJournalSelect/" + this.selectedProject)
        .then((res) => {
    ...
    chooseBuilding() {
      axios
        .get("/api/fetchReceiptJournalByBuildingName/" + this.selectedBuilding)
        .then((res) => {
          this.receipt_journals = res.data;
        })
        .catch((err) => {
