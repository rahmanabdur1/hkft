<template>
  <div>
    <h2 class="text-center mb-4 hkf_title">Purchase Journal</h2>
        ...
        <tbdoy v-for="(purchased_journal, index) in purchased_journals" :key="index">
          ...
          <tr style="padding: 1rem; border: 1px solid black; border-collapse: collapse">
            ...
            <td
              ...
              "
            >
              {{ purchased_journal.product_name }}
            </td>
            <td
              style="
                padding: 1rem;
            ...
            <td
              ...
              "
            >
              {{ parseFloat(purchased_journal.total_amount).toFixed(2) }}
            </td>
            <td
              style="
                padding: 1rem;
          ...
          <tr style="padding: 1rem; border: 1px solid black; border-collapse: collapse">
            ...
            <td
              style="
                ...
                width: 24%;
              "
              v-if="purchased_journal.bank != null"
            >
              Bank
            </td>
            <td
              style="
                ...
                width: 24%;
              "
              v-if="purchased_journal.cash != null"
            >
              Cash
            </td>
            <td
            ...
            <td
              ...
              "
            >
              {{ parseFloat(purchased_journal.paid).toFixed(2) }}
            </td>
          </tr>
          <tr style="padding: 1rem; border: 1px solid black; border-collapse: collapse">
            <td
...
export default {
  ...
  beforeRouteEnter(to, from, next) {
    next((vm) => {
      ...
      if (!User.hasLoggedIn()) {
        ...
        vm.$router.push({ name: "purchase_journal" });
      }
    });
  },
  data() {
  ...
  methods: {
    dateAccounts() {
      ...
      axios
        .post("/api/purchased_journal/daily_journal", data, {
          headers: {
    ...
    showAll() {
      axios
        .get("/api/purchased_journal")
        .then((res) => {
          const purchase = res.data;
          this.empty = false;
          this.purchased_journals = [];
          purchase.forEach((item) => {
            if (parseInt(item.total_amount) == parseInt(item.paid)) {
              this.purchased_journals.push(item);
            }
          });
        })
  ...
  computed: {
    selectDate() {
      axios
        .get("/api/purchased_journal/daily_journal/" + this.sendDate)
        .then((res) => {
          this.purchased_journals = [];
          this.empty = false;
          res.data.forEach((item) => {
    ...
    chooseProject() {
      ...
      axios
        .get("/api/fetchPurchasedJournal/" + this.selectedProject)
        .then((res) => {
    ...
    chooseBuilding() {
      axios
        .get("/api/fetchByBuildlingsInPurchaseJournal/" + this.selectedBuilding)
        .then((res) => {
          this.purchased_journals = res.data;
        });
    },
