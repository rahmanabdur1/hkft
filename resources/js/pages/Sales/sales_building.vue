<template>
          ...
          <tr v-for="(project,index) in filterSearch" :key='index'>
            ...
            <td class="remove_bg_td">
              <router-link
                ...
                :to="{
                  name: 'sales_project_report',
                  params: { id: project.building_name },
                }"
                >View</router-link
              >
            </td>
  ...
  export default {
          ...
          axios
            ...
            .then((res) => {
              ...
              res.data.forEach((item) => {
                promises.push(
                  axios
                    .get("/api/fetch_sales_project_manager/" + item.project)
                    .then((res) => {
                      return res.data;
                    })
                );
    ...
    created() {
      this.fetchProjects("/api/fetch_sales_building/"+this.$route.params.id);
    },
    computed: {
      filterSearch() {
    ...
    beforeRouteEnter(to, from, next) {
      next((vm) => {
        ...
        if (!User.hasLoggedIn()) {
          ...
          vm.$router.push({ name: "sales_building" });
        }
      });
    },
  };
