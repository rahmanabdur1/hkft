<template>
  <div>
    <h2 class="hkf_title text-center mb-4">Clients Due Clear Lists</h2>
    <div class="row">
      <div class="input-group col-md-6 page_to_page">
        <router-link :to="{ name: 'pay_receivable' }" class="btn btn-primary"
        ...
        <tr v-for="(payable,index) in filterSearch" :key='index'>
          ...
          <td class="remove_bg_td">
            <router-link
              class="action btn"
              style="width: 70px"
              :to="{ name: 'edit_pay_receivable', params: { id: payable.id } }"
              >Update</router-link
            >
          </td>
        </tr>
...
export default {
        ...
        axios
          ...
          .then((res) => {
            res.data.forEach((item) => {
              axios.get("/api/getManagerReceivableAll/" + item.project).then((res) => {
                res.data.forEach((item) => {
  ...
  created() {
    this.fetchApledgers("/api/getApledgerRcpt");
  },
  computed: {
    filterSearch() {
      return this.payables.filter((payable) => {
        const search_Term = this.searchTerm.toLowerCase();
        const typeName = payable.receipt_id.toLowerCase();
  ...
  beforeRouteEnter(to, from, next) {
    next((vm) => {
      ...
      if (!User.hasLoggedIn()) {
        ...
        vm.$router.push({ name: "all_pay_receivable" });
      }
    });
  },
};
