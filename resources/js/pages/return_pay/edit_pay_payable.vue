            ...
            <div class="col-md-5 formBox depositorBox">
              ...
              <div class="form-group mx-sm-3 mb-2">
                <label class="hkf_text"
                  >Pay Amount (previous paid amount + current amount )</label
                >
                <input
                  type="text"
                  class="form-control"
              ...
              <div class="form-group mx-sm-3 mb-2 two_group">
                <div>
                  ...
                  <span class="hkf_text" style="margin-right: 2px"
                    >{{ form.second_payment_due }} tk</span
                  >
                </div>
              </div>
              <div class="form-group mx-sm-3 mb-2 two_group">
...
export default {
  ...
  beforeRouteEnter(to, from, next) {
    next((vm) => {
      ...
      if (!User.hasLoggedIn()) {
        ...
        vm.$router.push({ name: "edit_pay_payable" });
      }
    });
  },
  ...
  created() {
    ...
    axios.get("/api/pay_payable").then((res) => {
      this.payments = res.data;
    });

    axios
      .get("/api/pay_payable/" + this.$route.params.id)
      .then((res) => {
        this.form = res.data;
  ...
  methods: {
    receiveVoucher() {
      axios
        .patch("/api/pay_payable/" + this.$route.params.id, this.form, {
          headers: {
    ...
    changeTotal() {
      ...
      let payment_due = this.form.payment_due;
      let totalAmount =
        -parseInt(this.tempAmount) + parseInt(current_amount) + parseInt(receive_amount);
      this.form.receive_amount = totalAmount;

      if (totalAmount == payment_due) {
        this.showSubmit = true;
        this.form.second_payment_due = 0;
        this.excess_receive = false;
      } else if (payment_due > totalAmount) {
        this.showSubmit = true;
        this.form.second_payment_due = payment_due - totalAmount;
        this.excess_receive = false;
      } else {
        this.showSubmit = false;
        this.form.second_payment_due = 0;
