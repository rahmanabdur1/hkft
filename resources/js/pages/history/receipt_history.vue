...
export default {
        ...
        axios
          ...
          .then((res) => {
            ...
            res.data.forEach((item) => {
              promises.push(
                axios.get("/api/getManagerReceiptHistory/" + item.project).then((res) => {
                  return res.data;
                })
              );
            });
  ...
  created() {
    this.fetchProjects("/api/fetch_client_history");
  },
  computed: {
    filterSearch() {
      return this.members.filter((project) => {
        const search_Term = this.searchTerm.toLowerCase();
        const typeName = project.receipt_id.toLowerCase();
  ...
  beforeRouteEnter(to, from, next) {
    next((vm) => {
      ...
      if (!User.hasLoggedIn()) {
        ...
        vm.$router.push({ name: "receipt_history" });
      }
    });
  },
};
