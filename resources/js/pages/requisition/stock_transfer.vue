...
export default {
  ...
  created() {
    if (!User.hasLoggedIn()) {
      ...
      this.$router.push({ name: "stock_transfer" });
    }
    axios
      .get("/api/stock_journal/" + this.$route.params.id)
      .then((res) => {
        console.log(res.data);
        this.form.total_quantity = res.data.quantity;
        this.form.product_name = res.data.product_name;
        this.form.fromProject = res.data.project;
        this.form.fromBuilding = res.data.selectedBuilding;
        this.form.unit = res.data.item_unit;
        this.form.payment_id = res.data.payment_id;
        this.form.item_id = res.data.item_id;
      })
      .catch((err) => {
  ...
  methods: {
    ...
    addAccountHead() {
      ...
      formData.append("transferQuantity", this.form.transferQuantity);
      ...
      axios
        .post("/api/stock_transfer", formData, {
          headers: {
  ...
  computed: {
    calculate() {
      if (parseInt(this.form.total_quantity) < parseInt(this.form.transferQuantity)) {
