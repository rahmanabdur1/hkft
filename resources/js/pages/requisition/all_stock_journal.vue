<template>
  <div>
    <h2 class="hkf_title text-center mb-4">All Stock Valuation</h2>
    <div class="row">
      <div class="input-group col-md-6 page_to_page">
        <router-link :to="{ name: 'add_stock_journal' }" class="btn btn-primary"
          >Add Stock Valuation</router-link
        >
      </div>
    ...
    <table class="table">
      ...
      <tbody>
        <tr v-for="(account_head, index) in filterSearch" :key="account_head.id">
          ...
          <td>{{ account_head.building_name }}</td>

          <td>{{ account_head.product_name }}</td>

          <td>
            {{ account_head.consumeQuantity }}
          </td>
          <td>
          ...
          <td class="remove_bg_td">
            <router-link
              class="btn"
              :to="{ name: 'edit_stock_journal', params: { id: account_head.id } }"
...
export default {
  ...
  methods: {
    sendDate() {
      ...
      axios
        .post("/api/fetchStockJournalByDate", requestData, {
          headers: {
    ...
    sendAll() {
      ...
      axios
        .post("/api/fetchStockJournalByDateProduct", data, {
          headers: {
            "Content-Type": "application/json",
            Accept: "application/json",
          },
  ...
  computed: {
    ...
    chooseProject() {
      ...
      axios
        .get("/api/fetchStockByProject/" + this.selectedProject)
        .then((res) => {
          this.account_heads = res.data;
        })
        .catch((err) => {
    ...
    chooseBuilding() {
      axios
        .get("/api/fetchStockByBuilding/" + this.selectedBuilding)
        .then((res) => {
          this.account_heads = res.data;
        })
        .catch((err) => {
  ...
  beforeRouteEnter(to, from, next) {
    next((vm) => {
      ...
      if (!User.hasLoggedIn()) {
        ...
        vm.$router.push({ name: "all_stock_journal" });
      }
    });
  },
};
