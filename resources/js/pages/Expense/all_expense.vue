<template>
  <div>
    <h2 class="hkf_title text-center mb-4">Expenses</h2>
    <div class="row">
      <div class="col-md-6 page_to_page">
        <router-link :to="{ name: 'add_expense' }" class="btn btn-primary"
          >Add Expense</router-link
        >
      </div>
      <div class="input-group col-md-6" style="display: block">
        <div style="display: flex">
          <input
            type="text"
            class="form-control status"
            placeholder="Expense Name"
            v-model="client_search"
            @keyup="listDisplay"
            @mouseup="removeList"
          />

          <button
            type="button"
            @click="searchByAll"
            class="btn btn-primary"
            style="height: 38px"
          >
            Search
          </button>
        </div>
        <div>
          <ul class="ulShow" :class="{ listStyle: list_style }">
            <li
              class="listShow"
              :class="{ listStyle: list_style }"
              v-for="(item, index) in filterSearch"
              :key="index"
              :value="item.product_name"
              @click="assignToSearch(item.product_name)"
            >
              {{ item.product_name }}
            </li>
          </ul>
        </div>
      </div>
    </div>
    <div class="row">
      <div class="form-group col-md-3 mx-sm-3 mb-2" v-if="admin">
        <label for="projectName" class="hkf_text">Select Project</label>
        <select
          class="form-control status"
          id="status"
          v-model="selectedProject"
          onfocus="this.size=10"
          onblur="this.size=1"
          onchange="this.size=1;this.blur();"
          @change="chooseProject"
          required
        >
          <option
            v-for="(project, index) in fetchprojects"
            :key="index"
            :value="project.selectedProject"
          >
            {{ project.selectedProject }}
          </option>
        </select>
      </div>
      <div class="form-group col-md-3 mx-sm-3 mb-2" v-if="manager">
        <label for="projectName" class="hkf_text">Select Project</label>
        <select
          class="form-control status"
          id="status"
          v-model="selectedProject"
          onfocus="this.size=10"
          onblur="this.size=1"
          onchange="this.size=1;this.blur();"
          @change="chooseProject"
          required
        >
          <option
            v-for="(project, index) in fetchprojects"
            :key="index"
            :value="project.project"
          >
            {{ project.project }}
          </option>
        </select>
      </div>
      <div class="form-group col-md-4 mx-sm-3 mb-2" v-if="isBuilding">
        <label for="projectName" class="hkf_text">Select Building</label>
        <select
          class="form-control status"
          id="status"
          v-model="selectedBuilding"
          onfocus="this.size=10"
          onblur="this.size=1"
          onchange="this.size=1;this.blur();"
          @change="chooseBuilding"
          required
        >
          <option
            v-for="(project, index) in buildings"
            :key="index"
            :value="project.building_name"
          >
            {{ project.building_name }}
          </option>
        </select>
      </div>
    </div>

    <div class="row">
      <form @submit.prevent="sendDate" class="col-md-12">
        <div class="row">
          <div class="form-group col-md-5">
            <label for="" class="hkf_text">Start Date</label>
            <input
              type="date"
              v-model="okStartDate"
              class="form-control rounded"
              aria-label="Search"
              aria-describedby="search-addon"
              required
            />
          </div>
          <div class="form-group col-md-5">
            <label for="" class="hkf_text">End Date</label>
            <input
              type="date"
              v-model="okEndDate"
              class="form-control rounded"
              aria-label="Search"
              aria-describedby="search-addon"
              required
            />
          </div>
          <div class="form-group col-md-2" style="margin-top: 30px">
            <button type="submit" class="btn btn-primary" style="height: 38px">
              Search
            </button>
          </div>
        </div>
      </form>
    </div>
    <table class="table">
      <thead>
        <tr>
          <th>Sl</th>
          <th>Expense Voucher</th>
          <th>Cost Name <i class="fa fa-sort" @click="sorting_asc"></i></th>
          <th>Date</th>
          <th>Project</th>
          <th>Building</th>
          <th>Payment Type</th>
          <th>Cost Amount</th>
          <th>Action</th>
        </tr>
      </thead>
      <tbody>
        <tr v-for="(project, index) in filterSearch" :key="index">
          <td>{{ index + 1 }}</td>
          <td>{{ project.expense_voucher }}</td>
          <td>{{ project.product_name }}</td>
          <td>{{ project.basic_date }}</td>
          <td>{{ project.selectedProject }}</td>
          <td>{{ project.building_name }}</td>
          <td>{{ project.selectedPaymentType }}</td>
          <td>{{ project.receive_amount }} Tk</td>

          <td class="remove_bg_td">
            <router-link
              class="btn"
              style="width: 4em"
              :to="{ name: 'edit_expense', params: { id: project.id } }"
              ><i class="fa fa-edit"></i
            ></router-link>
            <router-link
              class="btn ml-2"
              :to="{ name: 'view_expense', params: { id: project.id } }"
              ><i class="fa fa-eye"></i
            ></router-link>
            <button type="button" class="btn ml-2" @click="deleteExpense(project.id)">
              <i class="fa fa-trash"></i>
            </button>
          </td>
        </tr>
        <tr>
          <td>Total:</td>

          <td></td>
          <td></td>
          <td></td>
          <td></td>
          <td></td>
          <td>{{ totalAmount }} Tk</td>

          <td></td>
        </tr>
        <!-- add more rows as needed -->
      </tbody>
    </table>
  </div>
</template>
<script>
export default {
  name: "all_expense",
  data() {
    return {
      selectedBuilding: "",
      projects: [],
      buildings: [],
      sorting: false,
      searchTerm: "",
      startDate: "",
      endDate: "",
      okStartDate: "",
      okEndDate: "",
      total_expense: "",
      temps: [],
      fetchprojects: [],
      selectedProject: "",
      isBuilding: false,
      list_style: true,
      client_search: "",
      totalAmount: 0,
    };
  },
  beforeRouteEnter(to, from, next) {
    next((vm) => {
      // Access the $router property here
      if (!User.hasLoggedIn()) {
        vm.$router.push({ name: "home" });
      } else {
        vm.$router.push({ name: "all_expense" });
      }
    });
  },
  methods: {
    listDisplay() {
      this.list_style = false;
    },
    removeList() {
      this.list_style = true;
    },
    assignToSearch(item) {
      this.client_search = item;
      this.list_style = true;
    },
    searchByAll() {
      if (User.getGuard() == "admin") {
        const info = {
          product_name: this.client_search,
          project: this.selectedProject,
          building: this.selectedBuilding,
          startDate: this.okStartDate,
          endDate: this.okEndDate,
        };
        axios
          .post("/api/productBaseSearchExpense", info, {
            headers: {
              "Content-Type": "application/json",
              Accept: "application/json",
            },
          })
          .then((res) => {
            this.projects = res.data;
            this.totalAmount = 0;
            this.projects.forEach((item) => {
              this.totalAmount += parseInt(item.receive_amount);
            });
          })
          .catch((err) => {
            console.log(err.response);
          });
      } else {
        const info = {
          product_name: this.client_search,
          project: this.selectedProject,
          building: this.selectedBuilding,
          startDate: this.okStartDate,
          endDate: this.okEndDate,
        };
        axios
          .post("/api/productBaseSearchExpenseManager", info, {
            headers: {
              "Content-Type": "application/json",
              Accept: "application/json",
            },
          })
          .then((res) => {
            this.projects = res.data;
            this.totalAmount = 0;
            this.projects.forEach((item) => {
              this.totalAmount += parseInt(item.receive_amount);
            });
          })
          .catch((err) => {
            console.log(err.response);
          });
      }
    },
    sendDate() {
      const data = {
        startDate: this.okStartDate,
        endDate: this.okEndDate,
        selectedProject: this.selectedProject,
        selectedBuilding: this.selectedBuilding,
      };
      axios
        .post("/api/fetchAllExpenseByDate", data, {
          headers: {
            "Content-Type": "application/json",
            Accept: "application/json",
          },
        })
        .then((res) => {
          this.projects = res.data;
          this.totalAmount = 0;
          this.projects.forEach((item) => {
            this.totalAmount += parseInt(item.receive_amount);
          });
        })
        .catch((err) => {
          console.log(err.response);
        });
    },
    sorting_asc() {
      this.sorting = !this.sorting;
      if (this.sorting == true) {
        return this.projects.sort((a, b) => {
          const nameA = a.product_name.toUpperCase();
          const nameB = b.product_name.toUpperCase();
          if (nameA > nameB) {
            return -1;
          }
          if (nameA > nameB) {
            return 1;
          }
          return 0;
        });
      } else {
        return this.projects.sort((a, b) => {
          const nameA = a.product_name.toUpperCase();
          const nameB = b.product_name.toUpperCase();
          if (nameA < nameB) {
            return -1;
          }
          if (nameA < nameB) {
            return 1;
          }
          return 0;
        });
      }
    },
    deleteExpense(id) {
      Swal.fire({
        title: "Are you sure?",
        text: "You won't be able to revert this!",
        icon: "warning",
        showCancelButton: true,
        confirmButtonColor: "#3085d6",
        cancelButtonColor: "#d33",
        confirmButtonText: "Yes, delete it!",
      }).then((result) => {
        if (result.isConfirmed) {
          axios
            .delete("/api/expense/" + id)
            .then((res) => {
              this.projects = this.projects.filter((item) => {
                return item.id != id;
              });
              this.totalAmount = 0;
              this.projects.forEach((item) => {
                this.totalAmount += parseInt(item.receive_amount);
              });
            })
            .catch((err) => {
              console.log(err.response);
            });

          Swal.fire("Deleted!", "Your file has been deleted.", "success");
        }
      });
    },
    fetchProjects(url) {
      //admin  & manager
      this.guard = User.getGuard();
      if (this.guard == "admin") {
        this.admin = true;
        this.manager = false;

        axios
          .get(url)
          .then((res) => {
            this.projects = res.data;
            // console.log(this.projects);
            this.totalAmount = 0;
            this.projects.forEach((item) => {
              this.totalAmount += parseInt(item.receive_amount);
            });
            console.log(this.totalAmount);
          })
          .catch((error) => {});

        //
        axios
          .get("/api/fetchAdminExpense")
          .then((res) => {
            this.fetchprojects = res.data;
          })
          .catch((err) => {
            console.log(err.response);
          });
      } else if (this.guard == "manager") {
        this.manager = true;
        this.admin = false;
        axios.get("/api/fethProjectPurchaseManager/" + User.getUserName()).then((res) => {
          this.fetchprojects = res.data;
        });

        this.manager_username = User.getUserName();

        axios
          .get("/api/getManagerProject/" + this.manager_username)
          .then((res) => {
            const promises = [];

            res.data.forEach((item) => {
              promises.push(
                axios.get("/api/manager_fetch_expense/" + item.project).then((res) => {
                  return res.data;
                })
              );
            });

            Promise.all(promises).then((res) => {
              this.projects = res.flat();
              this.totalAmount = 0;
              this.projects.forEach((item) => {
                this.totalAmount += parseInt(item.receive_amount);
              });
            });
          })
          .catch((err) => {});
      }
      //end
    },
    totalExpense() {
      const requestData = {
        startDate: this.startDate,
        endDate: this.endDate,
      };
      axios
        .post("/api/expense/totalExpense", requestData, {
          headers: {
            "Content-Type": "multipart/form-data",
            Accept: "application/json",
          },
        })
        .then((res) => {
          this.total_expense = res.data.totalExpense;
        })
        .catch((err) => {
          console.log(err.response);
        });
    },
  },
  created() {
    this.fetchProjects("/api/expense");
  },
  computed: {
    filterSearch() {
      const voucher = this.projects.filter((project) => {
        const search_Term = this.client_search.toLowerCase();
        const typeName = project.product_name.toLowerCase();
        return typeName.match(search_Term);
      });

      this.totalAmount = 0;
      voucher.forEach((item) => {
        this.totalAmount += parseInt(item.receive_amount);
      });

      return voucher;
    },
    chooseProject() {
      this.isBuilding = true;
      axios
        .get("/api/fetchAdminExpenseByProject/" + this.selectedProject)
        .then((res) => {
          this.projects = res.data;

          axios
            .get("/api/fetchBuildingInventory/" + this.selectedProject)
            .then((res) => {
              this.buildings = res.data;
              console.log(this.buildings);
            })
            .catch((err) => {
              console.log(err.response);
            });
          this.totalAmount = 0;
          this.projects.forEach((item) => {
            totalAmount += parseInt(item.receive_amount);
          });
        })
        .catch((err) => {
          console.log(err.response);
        });
    },
    chooseBuilding() {
      axios
        .get("/api/expensebuilding/" + this.selectedBuilding)
        .then((res) => {
          this.projects = res.data;
          this.totalAmount = 0;
          this.projects.forEach((item) => {
            this.totalAmount += parseInt(item.receive_amount);
          });
        })
        .catch((err) => {
          console.log(err.response);
        });
    },
  },
};
</script>
<style>
.hkf_text {
  color: #4e3089 !important;
}
th {
  font-weight: bold;
  color: #473b5edb;
}
table tr:nth-child(even) {
  background-color: #605ca8 !important;
  color: white;
}
.action {
  color: white;
  background: #5d57c3;
  padding: 5px;
  width: 60px;
}
table tr:nth-child(odd) {
  color: #473b5edb;
  font-weight: bold;
}
.status_project {
  font-weight: bold;
  font-size: 1.2rem;
}
.text-warning {
  color: #35354c !important;
}
.text-info {
  color: #36becc !important;
}
.text-success {
  color: #66a756 !important;
}
.page_to_page {
  margin-bottom: 1rem;
}
table tr:nth-child(even) .remove_bg_td {
  background: white;
}
</style>
