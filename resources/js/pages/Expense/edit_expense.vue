<template>
  <div>
    <h2 class="text-center mb-4 hkf_title">Edit Expense</h2>
...
export default {
  ...
  beforeRouteEnter(to, from, next) {
    next((vm) => {
      ...
      if (!User.hasLoggedIn()) {
        ...
        vm.$router.push({ name: "edit_expense" });
      }
    });
  },
  data() {
  ...
  created() {
    ...
    //end
    axios
      .get("/api/expense/" + this.$route.params.id)
      .then((res) => {
        this.form = res.data;
        axios
          .get("/api/fetchBuildingByProjects/" + this.form.selectedProject)
          .then((res) => {
  ...
  methods: {
    ...
    receiveVoucher() {
      ...
      //payment_voucher
      axios
        .post("/api/expense/updateExpense", formData, {
          headers: {
