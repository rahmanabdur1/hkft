<template>
  <div>
    ...
    <div id="print-container3" class="mt-5">
      <div class='row'>
            <div class='col-md-12'>
                <img :src="'/' + form.nid_img" class='print_img' style='width:800px;height:auto;'/>
            </div>
  ...
  beforeRouteEnter(to, from, next) {
    next((vm) => {
      ...
      if (!User.hasLoggedIn()) {
        ...
        vm.$router.push({ name:"nid_client" });
      }
    });
  },
  created() {
    if(!User.hasLoggedIn()){
      ...
      this.$router.push({name:'nid_client'});
    }
    let id = this.$route.params.id;
    axios
      .get("/api/client/" + id)
      .then(({ data }) => {
        this.form.nid_img = data.nid_img;
