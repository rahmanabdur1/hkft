<template>
  <div>
    <h2 class="hkf_title text-center mb-4">Reference List</h2>
    <div class="row">
      <div class="input-group col-md-6 page_to_page">
        <router-link :to="{ name: 'add_reference' }" class="btn btn-primary"
          >Add Reference
        </router-link>
      </div>
    ...
    <table class="table">
      <thead>
        <tr>
          <th>Reference Name <i class="fa fa-sort" @click="sorting_asc"></i></th>
          <th>Reference ID</th>
          <th>Phone</th>
          <th>Action</th>
        </tr>
      ...
      <tbody>
        <tr v-for="reference in filterSearch" :key="reference.id">
          ...
          <td class="remove_bg_td">
            <router-link
              class="action btn"
              :to="{ name: 'edit_reference', params: { id: reference.id } }"
              >Edit</router-link
            >
            <router-link
              class="action btn ml-2"
              :to="{ name: 'view_reference', params: { id: reference.id } }"
              >View</router-link
            >
          </td>
        </tr>
    ...
    sorting_asc() {
      ...
      if (this.sorting == true) {
        return this.references.sort((a, b) => {
          const nameA = a.reference_name.toUpperCase();
          const nameB = b.reference_name.toUpperCase();
          if (nameA > nameB) {
            return 1;
          }
          if (nameA < nameB) {
  ...
  computed: {
    filterSearch() {
      return this.references.filter((reference) => {
        ...
        const typeName = reference.reference_name.toLowerCase();
  ...
  beforeRouteEnter(to, from, next) {
    next((vm) => {
      ...
      if (!User.hasLoggedIn()) {
        ...
        vm.$router.push({ name: "all_reference" });
      }
    });
  },
};
