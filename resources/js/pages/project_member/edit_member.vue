...
export default {
  ...
  beforeRouteEnter(to, from, next) {
    next((vm) => {
      ...
      if (!User.hasLoggedIn()) {
        ...
        vm.$router.push({ name: "edit_member" });
      }
    });
  },
  data() {
  ...
  created(){
    ...
    axios.get('/api/project_member/'+this.$route.params.id).then((res)=>{
        this.form = res.data;
    }).catch((err)=>{
        console.log(err.response);
    })
  ...
  methods: {
    addMember(){
        axios.patch('/api/project_member/'+this.$route.params.id,this.form).then((res)=>{
