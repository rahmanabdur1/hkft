<template>
  <div>
    <h2 class="hkf_title text-center mb-4">Employees</h2>
    <div class="row">
    ...
    <!-- <form @submit.prevent="totalExpense()" class="mt-5">
      ...
      <div class="col-md-6">
        <label for="" class="col-form-label hkf_text"
          >Total Salary (All Project Base): {{ this.total_salary }}</label
        >
      </div>
    </form> -->
      ...
      <tbody>
        <tr v-for="(project, index) in filterSearch" :key="index">
          ...
          <td class="remove_bg_td">
            <router-link
              class="action btn"
              :to="{ name: 'edit_employee', params: { id: project.id } }"
              >Edit</router-link
            >
            <router-link
              class="action btn ml-2"
              :to="{ name: 'view_employee', params: { id: project.id } }"
              >View</router-link
            >
            <router-link
              class="action btn ml-2"
              :to="{ name: 'pay_employee', params: { id: project.id } }"
              >Pay</router-link
            >
            <button
              type="button"
...
export default {
  ...
  beforeRouteEnter(to, from, next) {
    next((vm) => {
      ...
      if (!User.hasLoggedIn()) {
        ...
        vm.$router.push({ name: "all_employee" });
      }
    });
  },
  methods: {
        ...
        axios
          ...
          .then((res) => {
            ...
            res.data.forEach((item) => {
              promises.push(
                axios.get("/api/manager_fetch_employee/" + item.project).then((res) => {
                  return res.data;
                })
              );
            });
    ...
    totalExpense() {
      ...
      axios
        .post("/api/salary/totalSalary", requestData, {
          headers: {
        ...
        .then((res) => {
          this.total_salary = res.data.totalSalary;
        })
        .catch((err) => {
          console.log(err.response);
        });
  ...
  created() {
    this.fetchProjects("/api/employee");
  },
  computed: {
    filterSearch() {
    ...
    chooseProject() {
      ...
      axios
        .get("/api/fetchAdminEmployeeByProject/" + this.selectedProject)
        .then((res) => {
          this.projects = res.data;
          this.isBuilding = true;
          axios
    ...
    chooseBuilding() {
      axios
        .get("/api/fetchsinglebuildingemployee/" + this.selectedBuilding)
        .then((res) => {
          this.projects = res.data;
