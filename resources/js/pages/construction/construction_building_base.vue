    ...
    <div class="row">
      <div class="col-md-12 mx-auto" id="print-container">
        ...
        >
          ...
          <tbody>
            <tr v-for="(project, index) in filterMembers" :key='index'>
...
export default {
  ...
  created() {
    axios
      .get("/api/fetch_construction_project_accounts_building/" + this.$route.params.id)
      .then((res) => {
  ...
  beforeRouteEnter(to, from, next) {
    next((vm) => {
      ...
      if (!User.hasLoggedIn()) {
        ...
        vm.$router.push({ name: "construction_building_base" });
      }
    });
  },
};
