<template>
  <div>
    ...
    <div class="row">
      ...
      <div class="col-md-6 page_to_page">
        <router-link :to="{ name: 'merge_required_construction' }" class="btn btn-primary"
          >Group Construction Cost</router-link
        >
      </div>
...
export default {
          ...
          if (User.getGuard() == "admin") {
            axios.get("/api/project").then((res) => {
              ...
              let promises = this.allprojects.map((item) => {
                return axios
                  .get("/api/constructionOne/" + item.project_name)
                  .then((res) => {
            ...
            axios
              ...
              .then((res) => {
                ...
                let promises = this.allProjects.map((item) => {
                  return axios
                    .get("/api/constructionOne/" + item.project)
                    .then((res) => {
                      if (res.data && res.data.length > 0) {
                        return res.data;
                      } else {
  ...
  created() {
    this.fetchProjects("/api/construction");
  },
  computed: {
    filterSearch() {
  ...
  beforeRouteEnter(to, from, next) {
    next((vm) => {
      ...
      if (!User.hasLoggedIn()) {
        ...
        vm.$router.push({ name: "all_required_construction" });
      }
    });
  },
};
