          ...
          <td class="remove_bg_td">
            <router-link
              ...
              :to="{
                ...
                params: { id: project.selectedBuilding },
              }"
              >View</router-link
            >
          </td>
...
export default {
        ...
        axios
          ...
          .then((res) => {
            ...
            res.data.forEach((item) => {
              promises.push(
                axios
                  .get("/api/getManagerBuildingListConstruction/" + item.project)
                  .then((res) => {
  ...
  created() {
    this.fetchProjects("/api/construction_building_list");
  },
  computed: {
    filterSearch() {
      return this.members.filter((project) => {
        const search_Term = this.searchTerm.toLowerCase();
        const typeName = project.selectedBuilding.toLowerCase();
  ...
  beforeRouteEnter(to, from, next) {
    next((vm) => {
      ...
      if (!User.hasLoggedIn()) {
        ...
        vm.$router.push({ name: "construction_building_list" });
      }
    });
  },
};
